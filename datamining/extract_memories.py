...
# Import from local app folder
from app.tools import get_google_creds, get_email_body, get_google_creds, get_user_email
...
def download_from_gcs(bucket_name, source_blob, destination_file):
    ...
    try:
        ...
        if blob.exists():
            ...
            print(f"☁️  Downloaded checkpoint from GCS: {source_blob}")
            return True
    except Exception as e:
        print(f"⚠️  GCS Download failed: {e}")
    return False

def extract_facts_from_emails_parallel(max_to_process=100, max_workers=5):
    ...
    print(f"\n🚀 Starting GCP-Ready Fact Extraction", flush=True)
    print(f"🔍 Filter: {SEARCH_QUERY}", flush=True)
    print(f"📦 Target: {max_to_process} emails", flush=True)
    print("-" * 50, flush=True)
    ...
    checkpoint_blob = f"checkpoint_{email}.json"

    # 0. Sync checkpoint from GCS to support resuming in serverless environment
    if gcs_bucket:
        download_from_gcs(gcs_bucket, checkpoint_blob, processed_ids_file)
    ...
    # Load processed IDs (now locally synced from GCS)
    processed_ids = set()
